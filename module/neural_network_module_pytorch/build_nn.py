...
def generate_data(config, n_samples, target_distribution=[], opt=1):
    ...
    if opt == 1:
        ...
        data = np.random.uniform(-20, 20, (n_samples, n_sources)).astype(data_type)
    elif opt == 5:
        data = np.random.randn(n_samples, n_sources).astype(data_type)
...
def train_model_and_save(model, n_samples_in, n_training_steps, model_path='model/model.pth', threshold=0, bias=4, n_steps_reevaluate=10000, bias_max=10, min_sampling = 1000, max_sampling=float("inf")):
    """main function for training
    
    Parameters:
        - model : an instance of NeuralNetwork
        - n_samples_in : the number of samples for training. If None, the number of samples will adapt as a function of the loss
        - n_training_steps : the number of training steps maximal
        ...
        for epoch in t:
            ...
            if loss < model.best_dist:
                ...
                if n_epochs_no_update > n_steps_reevaluate:
                    model.best_dist = evaluate_model_return_dist(model.n_samples, model, model_path, model.dist)
                    n_epochs_no_update = 0
...
def plot_strats(model, party, source_x, source_y, n_points=1000, set_sources_value={}, uniform_local_variables=True, plot_most_likely_outcomes=False):
    ...
    if opt==1:
        ...
        mu, sigma = -0.5/0.1443375673, 1/0.1443375673
